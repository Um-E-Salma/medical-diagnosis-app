...
def main():
    ...
    with mlflow.start_run() as run:
        try:
            ...
            # Load trained model
            model = load_model(os.path.join(root_dir, 'rf_model.pkl'))
            ...
            # Infer signature
            input_example = pd.DataFrame(X_test[:5], columns=test_data.drop("HeartDisease", axis=1).columns)
            ...
            log_confusion_matrix(cm, "HeartDisease_Test")

            # Add tags
            mlflow.set_tag("model_type", type(model).__name__)
            mlflow.set_tag("task", "Heart Disease Prediction")
            mlflow.set_tag("dataset", "Heart Disease Dataset")
